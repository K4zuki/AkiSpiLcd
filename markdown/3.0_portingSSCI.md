...
| モード  |                   ビットフィールド                    |
|:-------:|:-----------------------------------------------------:|
|  3-bit  | ![](images/bitfield16/jditransfer3bit.png){width=90%} |
| \\hline |                                                       |
|  1-bit  | ![](images/bitfield16/jditransfer1bit.png){width=90%} |
| \\hline |                                                       |
|  4-bit  | ![](images/bitfield16/jditransfer4bit.png){width=90%} |
...
graph BT
    ...
    SM(SsciLCD_MODE) ==> LM(LCD_MODE)
```

## 小さな液晶、小さな基板でいいじゃない

![スイッチサイエンスメモリ液晶ブレークアウト基板（の図）](images/sscipcb.png){width=60%}

スイッチサイエンス(JDI)液晶用の基板を設計しました。液晶が小さいので基板も小さくし(40x35mm)、
ブレッドボードに載るようにピン間を1100milにしました。
...
基板の設計データはこちらのリポジトリに用意してあります。Eagleファイルと出力済ガーバデータです。
ガーバデータは小基板を4面付にしてあります。
