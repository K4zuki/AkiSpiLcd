...
graph TD
    ...
    MM{Mode Select M2}
    M0 -->|M0 = L: Function Mode|MM
    M3{Mode Select M3}
    MM -->|M2 = H: All Clear Flag|M3
    MM -->|M2 = L: No Update Mode|M3
    M3 --> T(M3 = L: Blinking OFF)
    M3 -->|M3 = H: Blinking ON|M5
    M5{Inversion Flag M5}
    M5 -->I(M5 = H: Inversion Display)
    M5 -->|M5 = L| M4{Color Flag M4}
    M4 -->BK(M4 = L: Black Display)
    M4 --> WH(M4 = H: White Display)
```
>>>>>>> Stashed changes
